...
Route::domain('jopos.' . $domain)->group(function () {
    ...
    Route::middleware(['auth', 'verified', 'employee'])->group(function () {
        ...
        Route::get('get-data/serial-number/produk', [SerialNumberController::class, 'getProduct'])->name(
            'serialNumber.getProduct',
        );
        ...
        // Kategori Produk
        Route::get('/kategoriproduk/{kategoriproduk}/json', [CategoryController::class, 'getKategoriJson'])->name(
            'kategoriproduk.getjson',
        );
        ...
        Route::post('/dashboard/kategoriproduk/upload', [CategoryController::class, 'upload'])->name(
            'kategoriproduk.upload',
        );
        Route::delete('/dashboard/kategoriproduk/revert', [CategoryController::class, 'revert'])->name(
            'kategoriproduk.revert',
        );
        ...
        // Garansi
        Route::get('/garansi/{garansi:slug}/json', [WarrantieController::class, 'getWarrantieJson'])->name(
            'garansi.getjson',
        );
        ...
        Route::get('/pembelian/{pembelian:referensi}/pdf', [PurchaseController::class, 'generatePdf'])->name(
            'pembelian.pdf',
        );
        Route::get('/pembelian/{pembelian:referensi}/thermal', [PurchaseController::class, 'printThermal'])->name(
            'pembelian.thermal',
        );
        // bayar hutang
        Route::post('pembelian/{pembelian}/payment', [PurchaseController::class, 'storePayment'])->name(
            'pembelian.payment.store',
        );
        ...
        Route::prefix('get-data')
            ...
            ->group(function () {
                ...
                Route::get('produk-by-barcode/{barcode}', [ProductController::class, 'getByBarcode'])->name(
                    'produk.by-barcode',
                );
                Route::get('serial-product-info/{produk}', [
                    SerialNumberController::class,
                    'getProductInfoForSerial',
                ])->name('serial-product-info');

                // Pengambilan Data Notifikasi
                Route::get('low-stock-notifications', [ProductController::class, 'getLowStockNotifications'])->name(
                    'notifications.low-stock',
                );
                Route::get('notifications/unregistered-serials', [
                    ProductController::class,
                    'getUnregisteredSerialNotifications',
                ])->name('notifications.unregistered-serials');
            });
    });
});
