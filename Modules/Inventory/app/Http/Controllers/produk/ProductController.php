...
class ProductController extends Controller implements HasMiddleware
{
    public static function middleware(): array
    {
        return [
            new Middleware(
                ...
                only: ['index', 'show', 'upload', 'revert', 'checkSlug'],
            ),
            new Middleware(
    ...
    public function store(Request $request)
    {
        ...
        \Log::info('Product store - gallery debug', [
            ...
            'gallery_sample' => collect($request->input('gallery', []))
                ->take(1)
                ->toArray(),
            'is_html' => collect($request->input('gallery', []))->contains(
                fn($v) => str_starts_with($v ?? '', '<!DOCTYPE'),
            ),
        ]);
        $request->validate([
            ...
            'variant_types.*.name' =>
                'required_with:variant_types|string|max:100',
            'variant_types.*.options' =>
                'required_with:variant_types|array|min:1',
            ...
            'variants.*.sku' =>
                'required_with:variants|string|unique:product_variants,sku',
            'variants.*.barcode' =>
                'nullable|string|unique:product_variants,barcode',
    ...
    public function edit(Product $produk)
    {
        return view('inventory::produk.edit', [
            ...
            'warranties' => Warrantie::where('status', 1)
                ->orderBy('name')
                ->get(),
            'pajak' => Taxe::orderBy('name_taxe', 'asc')->get(),
    ...
    public function update(Request $request, Product $produk)
    {
        ...
        $request->validate([
            ...
            'slug' => [
                'required',
                'string',
                Rule::unique('products', 'slug')->ignore($produk->id),
            ],
            'barcode' => [
                'nullable',
                'string',
                Rule::unique('products', 'barcode')->ignore($produk->id),
            ],
            'sku' => [
                'required',
                'string',
                Rule::unique('products', 'sku')->ignore($produk->id),
            ],
            'kategori' => 'required|exists:categories,id',
            'brand' => 'required|exists:brands,id',
            'unit' => 'required|exists:units,id',
    ...
    private function saveGallery(
        ...
        foreach ($allPaths as $tmpPath) {
            ...
            if (!$moved) {
                \Log::error(
                    "Failed to move gallery file: $tmpPath -> $newPath",
                );
                continue;
            }
            ...
            $isPrimary =
                (!$hasPrimary && $tmpPath === $primaryPath) ||
                (!$hasPrimary && $order === 1);
    ...
    private function saveVariants(
        ...
        foreach ($variantCombinations as $varData) {
            ...
            if ($tmpImg) {
                if (
                    str_starts_with($tmpImg, 'tmp/') &&
                    Storage::disk('r2')->exists($tmpImg)
                ) {
    ...
    public function upload(Request $request)
    {
        try {
            ...
            if ($path) {
                ...
                return response($path, 200)->header(
                    'Content-Type',
                    'text/plain',
                );
            }
                ...
                $q->where(function ($subQ) use ($search) {
                    $subQ
                        ...
                        ->orWhereHas('variants', function ($qv) use ($search) {
                            $qv->where('sku', 'like', "%{$search}%")->orWhere(
                                'barcode',
                                'like',
                                "%{$search}%",
                            );
        ...
        foreach ($products as $product) {
            ...
            if ($product->variants && $product->variants->count() > 0) {
                foreach ($product->variants as $variant) {
                    ...
                    $variantName = !empty($variantOptions)
                        ? implode(' / ', $variantOptions)
                        : 'SKU: ' . $variant->sku;
    ...
    public function getByBarcode(string $barcode)
    {
        ...
        return response()->json(
            ['message' => 'Product dengan barcode ini tidak ditemukan.'],
            404,
        );
    }
    ...
    public function getLowStockNotifications()
    {
        ...
        $lowStockProducts = Product::select('products.*')
            ...
            ->when(
                $storeId,
                fn($q) => $q->where('product_stocks.store_id', $storeId),
            )
        ...
        return response()->json([
            ...
            'products' => $lowStockProducts->map(function ($produk) {
                return [
                    ...
                    'qty' =>
                        $produk->stocks->firstWhere('product_variant_id', null)
                            ?->qty ?? 0, // Atau hitung via subquery
    ...
    {
        $subQueryLogic = fn($query) => $query->whereNotIn('status', [
            'Terjual',
            'Hilang',
        ]);
        ...
        $productsNeedingSerials = Product::where('wajib_seri', true)
            ...
            ->withCount([
                'serialNumbers as sn_tercatat_count' => function ($query) {
        ...
        return response()->json([
            ...
            'products' => $productsNeedingSerials->map(function ($produk) {
                return [
                    'name_product' => \Illuminate\Support\Str::limit(
                        $produk->name_product,
                        30,
                    ),
    ...
    public function lowStock(Request $request)
    {
        ...
        if ($request->filled('kategori')) {
            ...
            $kategoriIds = Category::where('id', $kategoriId)
                ->orWhere('parent_id', $kategoriId)
                ->pluck('id');
        ...
        if ($request->filled('supplier')) {
            ...
            $query->whereHas('latestPurchaseItem.pembelian', function ($q) use (
                $supplierId,
            ) {
        ...
        if ($request->ajax()) {
            return view(
                'inventory::inventaris.stock.partials.tabel-stok-rendah',
                compact('products'),
            )->render();
        }
