...
class ProductController extends Controller implements HasMiddleware
{
  ...
  public function create()
  {
    return view('inventory::produk.create', [
      'kategoris' => Category::where('status', 1)
        ->whereNull('parent_id')
        ->with([
          'children' => fn($q) => $q->where('status', 1)->orderBy('name'),
        ])
        ->orderBy('name')
        ->get(),
    ...
    $request->validate([
      ...
      'variants.*.sku'          => [
        ...
        function ($attribute, $value, $fail) use ($produk) {
          ...
          $exists = ProductVariant::where('sku', $value)->where('product_id', '!=', $produk->id)->exists();
  ...
  private function saveGallery(Product $product, array $galleryPaths, ?string $primaryPath): void
  {
    ...
    foreach ($allPaths as $tmpPath) {
      ...
      $isPrimary = (!$hasPrimary && $tmpPath === $primaryPath) || (!$hasPrimary && $order === 1);
  ...
  public function getData(Request $request)
  {
    ...
    $query = Product::with([
      ...
      ->when($request->boolean('hide_fulfilled'), function ($q) use ($storeId) {
        $q->withSum(
          [
            'stocks as total_stok' => function ($sq) use ($storeId) {
              if ($storeId) {
                $sq->where('store_id', $storeId);
              }
            },
          ...
          ->withCount([
            'serialNumbers as total_sn' => function ($sq) {
              $sq->where('status', 'Tersedia');
            },
          ])
  ...
  public function lowStock(Request $request)
  {
    $lastSaleDateSubquery = SaleItem::select('sales.tanggal_penjualan')
      ...
      ->where('sales.status_pembayaran', '!=', 'Batal')
      ->orderBy('sales.tanggal_penjualan', 'desc')
      ->limit(1);

    // FIX: Hapus whereNull('product_variant_id') agar total stok varian ikut terbaca
    $lowStockSubquery = ProductStock::selectRaw('COALESCE(SUM(qty), 0)')->whereColumn('product_id', 'products.id');

    // FIX: Tambahkan 'variants' dan 'stocks' ke dalam eager load
    $query = Product::with(['category', 'primaryImage', 'variants', 'stocks'])
      ->withTotalStock()
