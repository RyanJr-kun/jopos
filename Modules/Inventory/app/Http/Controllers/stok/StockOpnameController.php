...
class StockOpnameController extends Controller
{
  public function index(Request $request)
  {
    ...
    if ($request->filled('kategori') && $selectedToko) {
      ...
      if ($request->filled('subkategori')) {
        ...
        $childIds = Category::where('parent_id', $selectedKategori)->pluck('id')->push($selectedKategori);
      ...
      if ($request->filled('search')) {
        ...
        $query->where(function ($q) use ($search) {
          $q->where('name_product', 'like', "%{$search}%")->orWhere('sku', 'like', "%{$search}%");
        });
      }
  ...
  public function store(Request $request)
  {
    ...
    foreach ($validatedData['items'] as $item) {
      ...
      $stockQuery = ProductStock::where('store_id', $tokoId)->where('product_id', $item['product_id']);
  ...
  private function generateOpnameCode()
  {
    ...
    $lastOpname = StockOpname::where('kode_opname', 'like', $prefix . '%')
      ->latest('kode_opname')
      ->first();
  ...
  public function history(Request $request)
  {
    ...
    if ($request->filled('search')) {
      $search = $request->input('search');
      $query->where(function ($q) use ($search) {
        $q->where('kode_opname', 'like', "%{$search}%")->orWhereHas('user', fn($q_s) => $q_s->where('username', 'like', "%{$search}%"));
      });
    }
  ...
  public function show($kode_opname)
  {
    ...
    $stok_opname->load(['user', 'store', 'details.produk.unit', 'details.produk.primaryImage', 'details.produk.variants']);
