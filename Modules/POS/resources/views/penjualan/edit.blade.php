            ...
            <div class="row g-3 mb-4">
                ...
                <div class="col-12 col-md-6 col-lg-3">
                    ...
                    <input id="tanggal_jatuh_tempo" name="tanggal_jatuh_tempo" type="date"
                        ...
                        value="{{ old('tanggal_jatuh_tempo', $penjualan->tanggal_jatuh_tempo ? \Carbon\Carbon::parse($penjualan->tanggal_jatuh_tempo)->format('Y-m-d') : '') }}">
                        ...
                        <tbody class="border-top-0">
                            @foreach ($penjualan->items as $index => $detail)
                                @php
                                    ...
                                    $subtotal_item =
                                        $detail->harga_jual * $detail->jumlah - ($detail->diskon_item ?? 0);
                                ...
                                <tr data-row-id="{{ $rowId }}">
                                    ...
                                    <input type="hidden" name="items[{{ $index }}][jumlah]"
                                        class="item-qty-hidden" value="{{ $detail->jumlah }}">
                                    ...
                                    <input type="hidden" class="item-pajak-rate-hidden"
                                        value="{{ $pajak_rate }}">
                                        ...
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div>
                                                ...
                                                @if ($detail->product && $detail->product->wajib_seri && $detail->serialNumbers->isNotEmpty())
                                                    @php
                                                        $snList = $detail->serialNumbers
                                                            ->pluck('nomor_seri')
                                                            ->toArray();
                                                    @endphp
                                                    <small class="text-info d-block mt-1">
                                                        <i class="bx bx-barcode text-xs me-1"></i>SN:
                                                        {{ implode(', ', $snList) }}
                                                    </small>
...
<script type="module">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const invoiceOwnedSNs = [];
        $('.item-sn-hidden').each(function() {
            invoiceOwnedSNs.push($(this).val());
        });
        ...
        const openSerialNumberModal = (rowId, productId, variantId, productName, requiredQty, imgUrl,
            existingSerials = []) => {
            ...
            fetch(url.toString(), {
                ...
                .then(data => {
                    ...
                    data.serial_numbers.forEach(sn => {
                        ...
                        const isOwnedByThisInvoice = invoiceOwnedSNs.includes(sn.serial_number);
                        const isDisabled = sn.status !== 'Tersedia' && !isChecked && !
                            isOwnedByThisInvoice;
        ...
        waitForDependencies(function() {
            ...
            if (snConfirmBtn) {
                snConfirmBtn.addEventListener('click', () => {
                    ...
                    const row = $(
                        `#table-penjualan tbody tr[data-row-id="${tempRowDataForSN}"]`);
                    if (row.length > 0) {
                        ...
                        let snDisplayHtml =
                            `<small class="text-info d-block mt-1 sn-display"><i class="bx bx-barcode text-xs me-1"></i>SN: ${selected.join(', ')}</small>`;
                        ...
                        const newRow = $(
                            `#table-penjualan tbody tr[data-row-id="${tempRowDataForSN}"]`);
                        if (newRow.length > 0) {
                            // Tulis hidden inputs SN ke baris baru
                            let hiddenInputsHtml = '';
                            const nameAttr = newRow.find('.item-qty-hidden').attr('name');
                            const match = nameAttr ? nameAttr.match(/items\[(\d+)\]/) : null;
                            const index = match ? match[1] : itemCounter - 1;

                            selected.forEach(sn => {
                                hiddenInputsHtml +=
                                    `<input type="hidden" name="items[${index}][serial_numbers][]" class="item-sn-hidden" value="${sn}">`;
                            });
                            newRow.find('.item-sn-hidden').remove();
                            newRow.append(hiddenInputsHtml);

                            let snDisplayHtml =
                                `<small class="text-info d-block mt-1 sn-display"><i class="bx bx-barcode text-xs me-1"></i>SN: ${selected.join(', ')}</small>`;
                            newRow.find('.sn-display').remove();
                            newRow.find('.item-name').parent().append(snDisplayHtml);
                        }
                    }
            ...
            $("#btn-add").on("click", function() {
                ...
                if (wajibSeri) {
                    ...
                    const currentQty = parseInt(targetRow.find('.item-qty-hidden').val()) ||
                        qtyToAdd;

                    openSerialNumberModal(rowId, produkId, variantId, produkNama, currentQty,
                        imageUrl, existingSerials);
                }
            });
            ...
            function calculateGrandTotal() {
                ...
                const service = parseFloat($("#service-input").val()) ||
                    0; // ✅ FIX Bug #6: include service
                const ongkir = parseFloat($("#ongkir-input").val()) || 0;
                const diskonTambahan = parseFloat($("#diskon-tambahan").val()) || 0;
                const totalAkhir = subtotalKeseluruhan + totalPajakKeseluruhan + service -
                    diskonTambahan +
                    ongkir;
            ...
            $("#saveItemChangesBtn").on("click", function() {
                ...
                // Ambil value tanpa menyentuh Qty
                const newHarga = parseCurrency($("#edit-item-harga").val());
                const newDiskon = parseCurrency($("#edit-item-diskon").val());

                // 1. Update Hidden Input
                row.find(".item-harga-hidden").val(newHarga);
                row.find(".item-diskon-hidden").val(newDiskon);

                // 2. Update Tampilan Teks di Tabel (Ini yang sebelumnya kurang)
                ...
                // 4. Kalkulasi Ulang Baris & Grand Total
                calculateRow(row);
                calculateGrandTotal();
            ...
            if (paymentModalElement) {
                ...
                paymentModalElement.addEventListener('show.bs.modal', function() {
                    ...
                    document.getElementById('payment-modal-total-edit').innerText =
                        formatCurrency(
                            total);
                ...
                const paymentRadios = document.querySelectorAll(
                    '#paymentModalEdit input[name="metode_pembayaran"]');
