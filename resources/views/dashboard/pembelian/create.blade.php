...
@section('breadcrumb')
    @php
        $breadcrumbItems = [
            ...
            ['name' => 'Buat Transaksi Pembelian', 'url' => route('pembelian.create')],
        ];
    @endphp
            ...
            <div class="card-body px-4 pt-0">
                <div class="row g-2">
                    <div class="col-md-3">
                        ...
                        <div class="d-flex">
                            <select class="form-select me-2 @error('pemasok_id') is-invalid @enderror" name="pemasok_id"
                                id="Pemasok" required>
                                <option value="" disabled selected>Pilih Pemasok</option>
                                @foreach ($pemasok as $item)
                                    <option value="{{ $item->id }}" @selected(old('pemasok_id') == $item->id)>{{ $item->nama }}
                                    </option>
                                @endforeach
                            </select>
                            @error('pemasok')
                            ...
                            <button type="button" class="btn btn-outline-info btn-xs mb-0" data-bs-toggle="modal"
                                data-bs-target="#createPemasokModal" title="Tambah Pemasok Baru">
        ...
        $(document).ready(function() {
            ...
            function calculateRow(row) {
                ...
                row.find(".subtotal-item").text(formatCurrency(
                    subtotalDenganPajak)); // Tampilkan subtotal setelah pajak
            }
                    ...
                    .catch(error => {
                        if (error.errors) {
                            ...
                            Object.keys(error.errors).forEach(key => {
                                const input = createPemasokForm.querySelector(
                                    `[name="${key}"]`);
                                if (input) {
